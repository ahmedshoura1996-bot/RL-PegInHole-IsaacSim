...
for step in range(NUM_STEPS):
    ...
    with torch.inference_mode():
        ...
        # =============================================================
        # M7.4 OBSERVATION CORRUPTION TRACE
        # Trace policy input BEFORE PPO inference.
        # =============================================================
        if 95 <= step <= 115:
            policy_obs_trace = obs["policy"]

            robot_trace = manager_env.scene["robot"]
            joint_pos_trace = robot_trace.data.joint_pos
            joint_vel_trace = robot_trace.data.joint_vel

            # Observation layout from the current environment:
            # 9 joint positions
            # 9 joint velocities
            # 3 object-position values
            # 3 target-position values
            # 8 previous-action values
            #
            # Total = 32. The policy tensor has 36 dimensions,
            # therefore print the complete vector rather than assuming
            # undocumented indices for the remaining dimensions.

            print()
            print("M7.4 OBSERVATION CORRUPTION TRACE")
            print("----------------------------------------------")
            print(f"Step: {step + 1}")

            print(
                "Policy obs max abs:",
                policy_obs_trace.abs().max().item()
            )

            print(
                "Policy obs mean abs:",
                policy_obs_trace.abs().mean().item()
            )

            print(
                "Actual J7 position:",
                joint_pos_trace[0, 6].item()
            )

            print(
                "Actual J7 velocity:",
                joint_vel_trace[0, 6].item()
            )

            print(
                "Policy observation [0]:",
                policy_obs_trace[0].detach().cpu().numpy()
            )
        ...
        if step >= 95 and step <= 130:
            print()
            print("M7.3 IK ACTION TRACE")
            print("----------------------------------------------")
            print(f"Step: {step + 1}")
