...
def main():
    ...
    # -------------------------------------------------------------------------
    # Gradient diagnostic hooks
    # -------------------------------------------------------------------------

    print("[DEBUG] Installing policy gradient hooks", flush=True)
        ...
        def make_hook(param_name):
            def hook(grad):
                ...
                if timestep >= 1070:
                    ...
                    max_abs = grad.detach().abs().max().item()

                    print(
                         f"[GRAD] update={timestep} "
                        f"policy.{param_name} "
                        f"finite={finite} "
    ...
    # Track the SAC update number
    _original_update = SAC.update
    ...
    def diagnostic_update(self, timestep, timesteps):
        ...
        if timestep >= 1070:
            p = self.policy.log_std_parameter

            print(
                f"\n[DIAG] ===== SAC UPDATE {timestep} =====",
                flush=True,
            )

            print(
                f"[DIAG] log_std BEFORE: "
           f"finite={torch.isfinite(p).all().item()} "
                f"min={p.detach().min().item():.8f} "
                f"max={p.detach().max().item():.8f}",
                flush=True,
            )
        ...
        if timestep >= 1070:
            ...
            print(
                f"[DIAG] log_std AFTER: "
                f"finite={torch.isfinite(p).all().item()} "
                f"min={p.detach().min().item()} "
                f"max={p.detach().max().item()}",
                flush=True,
            )
