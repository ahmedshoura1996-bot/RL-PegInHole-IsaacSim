...
def install_forward_diagnostics(agent):
    """Print policy/critic forward outputs near the SAC instability."""
    ...
    def make_hook(name):
        def hook(module, inputs, output):
            timestep = getattr(agent, "_diag_timestep", -1)

            if timestep < 1084:
                return

            try:
                # Model.act can return tuples; keep this generic.
                value = output[0] if isinstance(output, tuple) else output

                if torch.is_tensor(value):
                    print(
                        f"[FORWARD] update={timestep} "
                        f"{name} "
                        f"finite={torch.isfinite(value).all().item()} "
                        f"min={value.detach().min().item():.6e} "
                        f"max={value.detach().max().item():.6e}",
                        flush=True,
                    )
            except Exception as exc:
                print(
                    f"[FORWARD] update={timestep} {name} ERROR: {exc}",
                    flush=True,
                )

        return hook

    agent.policy.register_forward_hook(make_hook("policy"))
    agent.critic_1.register_forward_hook(make_hook("critic_1"))
    agent.critic_2.register_forward_hook(make_hook("critic_2"))
...
def main():
    ...
    install_forward_diagnostics(agent)
